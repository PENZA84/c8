...
def get_channel_http(channel_url):
    try:
        ...
        # 存入统计列表
        with lock:
            channel_stats.append([channel_url, len(url_list), len(text_list)])
...
if __name__ == '__main__':
    ...
    with open('channel_stats.csv', 'w', encoding='utf-8-sig', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['频道URL', 'URL数量', '节点数量'])
